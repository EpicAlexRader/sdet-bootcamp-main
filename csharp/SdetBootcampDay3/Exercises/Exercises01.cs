...
namespace SdetBootcampDay3.Exercises
{
    ...
    public class Exercises01
    {
        ...
        public async Task GetDataForUser2_CheckContentType_ShouldContainApplicationJson()
        {
            ...
            Assert.That(responce.ContentType, Does.Contain("application/json"));
        }
        ...
        public async Task GetDataForUser3_CheckServerHeader_ShouldBeCloudflare()
        {
            ...
            // string serverHeaderValue = responce.Headers
            //     .Where(x => x.Name.Equals("Server"))
            //     .Select(x => x.Value)
            //     .First();
            Assert.That(responce.Server, Is.EqualTo("cloudflare"));
        }
        ...
        public async Task GetDataForUser4_CheckName_ShouldBePatriciaLebsack()
        {
            ...
            RestResponse responce = await client.ExecuteAsync(request);
            JObject responceData = JObject.Parse(responce.Content!);

            Assert.That(responceData.SelectToken("name")!.ToString(), Is.EqualTo("Patricia Lebsack"));
        }
        ...
        public async Task GetDataForUser5_CheckCompanyName_ShouldBeKeeblerLLC()
        {
            ...
            Assert.That(responceData.SelectToken("company.name")!.ToString(), Is.EqualTo("Keebler LLC"));
        }
    }
}
